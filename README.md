...
已有的 inactive worktree 仍带旧 Makefile 时，执行 `python scripts/production_cli.py materialize --rebuild-inactive`。它只接受 `NOT_SUBMITTED` 或 scratch 已清理的 `EXIT/QC_FAIL`，将旧 tree 保留到 `.production_runtime/superseded_worktrees/` 后再生成新 tree；活动、DONE 和未清理 scratch 的 run 会被拒绝。
